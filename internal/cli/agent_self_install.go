...
func bootstrapText(agent string) string {
	...
	fmt.Fprintln(&b)
	fmt.Fprintln(&b, "Use LogDiet from now on:")
	fmt.Fprintln(&b, "  logdiet wrap -- go test ./...")
	fmt.Fprintln(&b, "  logdiet wrap -- pytest -q")
	fmt.Fprintln(&b, "  logdiet wrap -- npm test")
	fmt.Fprintln(&b, "  logdiet wrap -- git diff")
	fmt.Fprintln(&b, "  logdiet wrap -- rg \"pattern\"")
	fmt.Fprintln(&b)
...
func agentInstructionsText(agent string) string {
	...
	fmt.Fprintln(&b, "For the rest of this session:")
	fmt.Fprintln(&b)
	fmt.Fprintln(&b, "- Use `logdiet wrap -- <command>` for noisy commands.")
